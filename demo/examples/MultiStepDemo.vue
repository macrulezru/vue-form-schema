    ...
    <div v-if="!submitted">
        ...
        <div style="display: flex; align-items: center; gap: 0">
          <template v-for="(step, i) in wizard.steps" :key="i">
            ...
            <div
              ...
              :style="{
                background: wizard.currentStep.value > i ? 'var(--accent)' : 'var(--border)',
              }"
            />
          </template>
        </div>
      ...
      <div class="card">
        <div class="card-title">
          Step {{ wizard.currentStep.value + 1 }}:
          {{ wizard.steps[wizard.currentStep.value].title }}
        </div>
...
:deep(.vfs-step-next:hover),
:deep(.vfs-step-submit:hover) {
  background: var(--accent-h);
}
:deep(.vfs-step-next:disabled),
:deep(.vfs-step-submit:disabled) {
  opacity: 0.45;
  cursor: not-allowed;
}
</style>
