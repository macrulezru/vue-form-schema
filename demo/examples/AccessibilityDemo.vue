  ...
  <div>
    ...
    <div class="card">
      ...
      <p style="font-size: 0.82rem; color: var(--muted); margin-bottom: 16px">
        Submit without filling the form to see errors announced via <code>role="alert"</code> and
        fields marked as <code>aria-invalid="true"</code>.
      </p>
...
&lt;div class="vfs-field vfs-field--text vfs-field--error"&gt;
...
const a11yItems = [
  {
    ...
    detail: 'Set to "true" on required inputs/selects/textareas/fieldsets',
  },
  ...
  {
    feature: 'role="alert" + aria-live="polite"',
    detail: 'Error messages are announced by screen readers on appearance',
  },
  {
    feature: 'label[for] + input[id]',
    detail: 'All inputs have matching label[for] and id attributes',
  },
  {
    feature: 'fieldset + legend for radio groups',
    detail: 'Radio groups are wrapped in <fieldset> with <legend> as the group label',
  },
  {
    feature: 'aria-checked on checkboxes',
    detail: 'Native checkboxes have explicit aria-checked reflecting the boolean value',
  },
]
</script>
...
:deep(.vfs-field--error .vfs-input),
:deep(.vfs-field--error .vfs-select) {
  border-color: var(--error);
}
:deep(.vfs-radio-group) {
  display: flex;
  flex-direction: column;
  gap: 10px;
}
:deep(.vfs-radio-label),
:deep(.vfs-checkbox-label) {
...
:deep(fieldset.vfs-field) {
  border: none;
  padding: 0;
  margin: 0 0 16px;
  min-width: 0;
}
:deep(fieldset.vfs-field legend) {
  padding: 0;
}
:deep(.vfs-submit) {
  display: inline-flex;
