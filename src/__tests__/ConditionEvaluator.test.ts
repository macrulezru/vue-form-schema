...
describe('ConditionEvaluator.evaluateFields', () => {
  ...
  it('does not execute arbitrary code (no eval / new Function)', () => {
    // Prior implementation ran expressions through `new Function`; this checks
    // that global scope / constructors are unreachable from an expression.
    const fields: FieldDefinition[] = [
      {
        type: 'text',
        name: 'bad',
        visible: '(values.constructor.constructor("return globalThis")())',
      },
    ]
    expect(() => evaluator.evaluateFields(fields, {})).not.toThrow()
    // constructor access is denylisted — resolves to undefined, not the real Object constructor
    expect(evaluator.evaluateFields(fields, {})[0].visible).toBe(true) // falls back to safe default
  })

  it('blocks __proto__ / prototype access', () => {
    const fields: FieldDefinition[] = [
      {
        type: 'text',
        name: 'x',
        visible: 'values.__proto__.polluted === true',
      },
    ]
    expect(evaluator.evaluateFields(fields, {})[0].visible).toBe(false)
  })

  it('supports arithmetic, comparison, logical and ternary operators', () => {
    const fields: FieldDefinition[] = [
      {
        type: 'text',
        name: 'x',
        visible: 'values.age >= 18 && values.age < 65 ? true : false',
      },
    ]
    expect(evaluator.evaluateFields(fields, { age: 30 })[0].visible).toBe(true)
    expect(evaluator.evaluateFields(fields, { age: 70 })[0].visible).toBe(false)
  })

  it('supports nested property and bracket access', () => {
    const fields: FieldDefinition[] = [
      {
        type: 'text',
        name: 'x',
        visible: "values.address['city'] === 'Moscow'",
      },
    ]
    expect(evaluator.evaluateFields(fields, { address: { city: 'Moscow' } })[0].visible).toBe(true)
    expect(evaluator.evaluateFields(fields, { address: { city: 'Berlin' } })[0].visible).toBe(false)
  })
