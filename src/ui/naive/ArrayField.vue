...
<template>
  <div style="margin-bottom: 1rem">
    <div v-if="field.label" style="margin-bottom: 0.5rem; font-weight: 500; font-size: 0.875rem">
      {{ field.label }}
    </div>

    <NCard v-for="row in rows" :key="row.key" size="small" style="margin-bottom: 0.5rem">
      <NFlex align="flex-start" justify="space-between">
        ...
        <NButton text type="error" size="small" @click="remove(row.index)">Remove</NButton>
      </NFlex>
    </NCard>

    <NButton dashed size="small" @click="append()">+ Add row</NButton>
  </div>
</template>
